...
class SearchControllerTest {
  ...
  void searchPosts_whenSizeIsInvalid_returnsBadRequest() throws Exception {
    mockMvc
        .perform(get("/v1/search/posts").param("size", "101"))
        .andExpect(status().isBadRequest());
  }

  @Test
  void getRegionCounts_returnsWrappedSuccessResponse() throws Exception {
    given(searchService.getRegionCounts(LocalDate.of(2026, 3, 1), LocalDate.of(2026, 3, 31)))
        .willReturn(
            List.of(
                new SearchRegionCountItem("서울/경기권", 128), new SearchRegionCountItem("충청권", 84)));
